...
function Label({ title }) {
  return <span className="text-base text-black">{title}</span>;
}

function TbRow({ label, children, mykey }) {
  return (
    <tr className="border-b text-base text-pColor" key={mykey}>
  ...
  return (
    <div className="absolute top-[45%] left-5 border-r-2 pr-2">
      ...
      <div
        className={`${
          open ? "" : "hidden"
        } absolute w-28 h-40 overflow-y-auto z-50 bg-white shadow-lg p-2 top-9 -left-3 rounded-md`}>
        {countrCode.map((data) => (
          <div
            key={data.name}
            className="flex items-center space-x-3"
            onClick={() => setMbCode(data) & setOpen(false)}>
